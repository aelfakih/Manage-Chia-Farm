...
def do_move_plots(style,mode):
    ...
    for line in results:
        ...
        free = bytes_to_gib ( free )
        total = bytes_to_gib ( total )
        row_entry = f'[{drive}]{print_spaces(drive,25)}| {nft:3.0f} NFTs, {og:3.0f} OGs | {free:5.0f}/{total:5.0f} ({free/total*100:5.2f})% GiB Free  |'
        if mode in "replace":
            if og == 0:
                from_drives.append (row_entry)
        else:
            from_drives.append(row_entry)
        if is_verbose ( ) :
            logging.info ( row_entry )
    ...
    if mode in "move":
        ...
        message = 'Select SOURCE location containing NFT plots, which will REPLACE OGs in farm?'
    ...
    if import_from == "Cancel":
        ...
        message = f"* Searching for .plot files in {import_from}"
        print(message)
        if is_verbose ( ) :
            logging.info ( f"Searching for .plot files in [%s] ..." % (import_from) )
        ...
        if mode in "move":
            message = 'INTO which Plot Directory you want to move plots TO?'
        else:
            message = 'INTO which Plot Directories you want to OVERWRITE OG PLOTS?'
